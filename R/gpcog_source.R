#' gpcog_source
#'
#' Add a source section to the bottom of a plot created with \code{gpcog_plot()}.
#'
...
#' @export
#'
gpcog_source <- function(text, size = 8, width = 132, plural = FALSE) {
  ...
  grob1 <- grid::textGrob(
    ...
    hjust = 0,
    vjust = 1,
    gp = gpar(fontsize = size, fontfamily = "Arial", fontface = "bold")
  )
  ...
  grob2 <- grid::textGrob(
    ...
    hjust = 0,
    vjust = 1,
    gp = gpar(fontsize = size, fontfamily = "Arial")
  )
  ...
  if (multiline) {
    ...
    grob3 <- textGrob(
      ...
      hjust = 0,
      vjust = 1,
      gp = gpar(fontsize = size, fontfamily = "Arial", lineheight = 1)
    )
